...
export default function UpdateGuestSpeaker() {
    var GuestSpeaker = reactLocalStorage.getObject('GuestSpeaker');
    const id = GuestSpeaker[0];
    const [name, setName] = useState(GuestSpeaker[1]);
    const [imageSelected, setimageSelected] = useState('https://res.cloudinary.com/applicationframework2021/image/upload/v1624901540/' + GuestSpeaker[2]);
    const [profession, setprofession] = useState(GuestSpeaker[3]);
    const [description, setdescription] = useState(GuestSpeaker[4]);
    const [status, setstatus] = useState(GuestSpeaker[5]);
    

    function updateGuestSpeaker(e) {
        e.preventDefault();
        ...
        axios.post("https://af-global-2021.herokuapp.com/guestSpeaker/update/" + id, updateGuestSpeaker).then(() => {
    ...
    return (
        <div>
            <div className="d-flex p-2" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '300px' }}>
                <h1 tag='div' className='display-1 pb-3 mb-3 border-bottom'>Addz Guest Speakers</h1>
            </div>
            <div className="col-md-14 col-sm-12" style={{ maxWidth: '70rem', margin: 'auto', padding: '10px' }}>
                <div className="row g-0">
                    ...
                    <button type="submit" onClick={updateGuestSpeaker} className="btn btn-primary btn-block mb-4">Save</button>
                    <a href="/viewguest">
